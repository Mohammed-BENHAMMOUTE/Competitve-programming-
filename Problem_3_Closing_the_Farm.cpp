...
void dfs(int source, vector<vector<int>> &adj, vector<bool> &added, vector<bool> &visited) {
    visited[source] = true;
    for(int next : adj[source]) {
        if(added[next] && !visited[next]) {
            dfs(next, adj, added, visited);
        }
    }
}

bool isConnected(int current, vector<vector<int>> &adj, vector<bool> &added) {
    vector<bool> visited(adj.size(), false);
    int cnt = 0;
    
    for(int i = 1; i < adj.size(); i++) {
        if(added[i] && !visited[i]) {
            cnt++;
            dfs(i, adj, added, visited);
        }
    }
    return cnt == 1;
}
...
void solve() {
    ...
    for(int current : order){
        added[current] = true;
        if(isConnected(current , adj  , added)){
            ans.pb("YES");
        }else{
            ans.pb("NO");
        }
