...
bool intersect(int b, int h, int y1 , int y2){
    return((b<=y2 && h>= y1) || (y1 <=h && y2>=b));
}

void solve() {
    int p,q,r,l;cin >> p >> q >>l >>r;
    ...
    for(int t = l; t <=r; t++){
        bool flag = false;
        for(auto x : vx){
            int b = x.first+t, h = x.second+t;
            for(auto y : vz) if(intersect(b,h,y.first, y.second)){
                flag =true;ans++;
                break;
            };
            // what(flag);
