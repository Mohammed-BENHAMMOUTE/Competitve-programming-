...
int dfs(int source, vi& subordinates, vvi& adj) {
    int count = 0;
    for (int child : adj[source]) {
        count += 1 + dfs(child, subordinates, adj);
    }
    subordinates[source] = count;
    return count;
}
...
void solve() {
    ...
    // Read the boss for each employee (2 to n)
    for (int i = 2; i <= n; i++) {
        int boss;
        cin >> boss;
    ...
    // Start DFS from employee 1 (the general director)
