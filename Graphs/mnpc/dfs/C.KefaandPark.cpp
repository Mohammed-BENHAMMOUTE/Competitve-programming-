...
void solve() {
  ...
  int answer = 0;
  std::function<void(int, int, int)> dfs = [&](int source, int num_cats, int parent) {
    if (cats[source] == 1){
      num_cats++;
      if (num_cats > m) {
        return;
    ...
    bool isLeaf = false;
    if (source == 1) {
      isLeaf = (adj[source].size() == 0);
    }else{
      isLeaf = (adj[source].size() == 1);
    }
