...
void dfs(vector<string>& mat , int i , int j , int n , int m, vector<vector<bool>>& visited ) {
    visited[i][j] = true;
    
    for (int x = 0; x < 4; x++) { 
        int ni = i + dx[x];
        int nj = j + dy[x];
        if(isValid(ni, nj , n ,m) && !visited[ni][nj] && mat[ni][nj] == '.') {
            dfs(mat, ni , nj , n, m, visited);
        }
    }
} 
