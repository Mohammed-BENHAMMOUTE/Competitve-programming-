...
void solve() {
    ...
    for(int k = 1; k <= mx; k++) {
        ...
        rep(i, 0, n) {
            ...
            ll larger = extra * (teams + 1);
            ll smaller = (k - extra) * teams;
            
            pr = larger * smaller + (extra * (extra-1)/2) * (teams+1) * (teams+1) + ((k-extra) * (k-extra-1)/2) * teams * teams;
