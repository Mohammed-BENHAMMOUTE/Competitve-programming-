    ...
    rep(i , 0 , n){
        if(!visited[i]){
            ...
            while(!q.empty()){
                ...
                minx = min(cord[current].first , minx);
                maxx = max(cord[current].first , maxx);
                miny = min(cord[current].second , miny);
                maxy = max(cord[current].second , maxy);
            ...
            minParamaeter = min(((maxx + maxy) - (minx + miny))*2 , minParamaeter);
        }
    }
    cout << minParamaeter << endl;
}
