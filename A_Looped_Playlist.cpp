...
void solve() {
    ...
    ll fullCycles = p/totalSum;
    ll remaining = p%totalSum;
    if(remaining == 0) {
        cout << 1 << " "<< n * fullCycles << endl;
        return;
    }
    ...
    {
        sum+= a[right];
        while(sum - a[left] >= remaining){
            sum -= a[left++];
        }
        if(sum >= remaining && (spread > (right-left+1))){
            index = left;
            spread = right - left +1;
        }
    }
    cout << index+1 << " " << spread + n * fullCycles << endl;
}
