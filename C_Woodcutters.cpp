...
void solve() {
    ...
    vector<vector<int>> dp(n, vector(3, INT_MIN));
    dp[0][0] = 0;
    dp[0][1] = (arr[0].first+arr[0].second < arr[1].first ? 1:0);
    dp[0][2] = 1;

    for(int i =1; i < n ;i++){
        dp[i]= dp[i-1];
        dp[i][0]= max({dp[i-1][0],dp[i-1][2],dp[i-1][1] });
        if(i < n-1 && arr[i].first+arr[i].second < arr[i+1].first){
            dp[i][1] = 1+max({dp[i-1][0],dp[i-1][2],dp[i-1][1] });
        }
        if(arr[i].first-arr[i].second > arr[i-1].first+arr[i-1].second){
            dp[i][2] = 1+max({dp[i-1][0],dp[i-1][2],dp[i-1][1] });
    ...
    cout<<max({dp[n-1][1], dp[n-1][2],dp[n-1][0]});
    return;
}
