...
void solve() {
    ...
    ll xs, ys , xt , yt;
    cin >> xs >> ys >> xt >> yt;
    long long mmDist = LLONG_MAX;
    for(int i =0 ;i < n ; i++) 
    {
        long long  x = pt[i].first;
        long long  y = pt[i].second;
        long long temp = (x-xt)*(x-xt)+ (y-yt)*(y-yt);
        mmDist = min(mmDist, temp);
    };
    
    long long fn = (xs -xt)*(xs -xt) + (ys-yt)*(ys-yt);
    string ans = ((fn < mmDist) ? "YES":"NO");
    cout <<ans << endl;
}
