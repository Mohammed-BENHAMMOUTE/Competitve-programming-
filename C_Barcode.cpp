...
void solve() {
    ...
    vector<string> mat(n);
    cinarr(mat);
    vector<vector<int>> pref(m + 1, vector<int>(2, 0));
    for (int i = 0; i < m; i++) {
        pref[i + 1][0] = pref[i][0];
        pref[i + 1][1] = pref[i][1];
        for (int j = 0; j < n; j++) {
            if (mat[j][i] == '.') pref[i + 1][0] += 1;
            if (mat[j][i] == '#') pref[i + 1][1] += 1;
        }
    }
    ...
    for (int i = 1; i <= m; i++) {
        for (int j = x; j <= y && i - j >= 0; j++) {
            dp[i][0] = min(dp[i][0], dp[i - j][1] + pref[i][1] - pref[i - j][1]);
            dp[i][1] = min(dp[i][1], dp[i - j][0] + pref[i][0] - pref[i - j][0]);
        }
    }
