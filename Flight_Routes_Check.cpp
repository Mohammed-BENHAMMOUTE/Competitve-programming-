...
void solve() {
    int n, m; cin >> n >> m;
    vvi adj(n), rev_adj(n);  // forward and reverse adjacency lists
    
    // Build both adjacency lists
    for(int i = 0; i < m; i++) {
        int u, v; cin >> u >> v; --u; --v;
        adj[u].pb(v);
        rev_adj[v].pb(u);  // reverse edge
    }

    // Check forward reachability from vertex 0
    ...
    // Check if all vertices are reachable forward
    for(int i = 0; i < n; i++) {
        if(!visited[i]) {
            cout << "NO\n" << "1 " << i+1 << "\n";
            return;
    ...
    while(!q.empty()) {
        int current = q.front(); q.pop();
        for(int neighbour : rev_adj[current]) {
            if(!visited[neighbour]) {
                visited[neighbour] = true;
                q.push(neighbour);
            }
    ...
    // Check if all vertices are reachable backward
    for(int i = 0; i < n; i++) {
        if(!visited[i]) {
            cout << "NO\n" << i+1 << " 1\n";
            return;
